...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let prompt = r#"Only answer to me in JSON. The format you should respond to me in is as follows: {"code": string, "description": string, "programming_language": string, "extension": string}. You can use the description field to give me some information about a command, this field should always be in the same language as the question the user is asking. You will be answering programming questions. Use the programming_language field to respond to what language the "code" output is in. Use the "extension" field to output the file extension for the code without the dot. Remember to handle newlines correctly in the JSON and escape any characters you need to. This is the user's question: "#;
    ...
    let mut ctx_file = std::fs::OpenOptions::new()
        .write(true)
        .truncate(true)
        .open(last_ctx_path)
        .expect("Unable to open context file for writing.");
    let out_json = serde_json::to_string(&body.context).unwrap();
    write!(ctx_file, "{}", out_json).expect("Unable to write to context file");

    let resp: ModelResponse =
        serde_json::from_str(&body.response).expect("Model responded with malformed JSON.");
    println!("Question:\n  {}", q);
    println!("Description:\n  {}", resp.description.green());
    if !resp.programming_language.is_empty() {
    ...
    if args.save {
        let ext = if resp.extension.is_empty() {
            ".txt"
        } else {
            &resp.extension
        };
